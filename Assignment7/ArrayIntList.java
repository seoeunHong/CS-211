 ...
 * Description: This class stores int value in itself and modifies or analyzes with its methods;
 * */
...
public class ArrayIntList implements Iterable<Integer> {
	...
	private void checkIndex(int index, int min, int max) {
		if (!(min <= index && index <= max)) {
			throw new ArrayIndexOutOfBoundsException("Illegal index " + index + "; must be between " + min + " and "
					+ max + "\n" + "list : " + toString() + " (size=" + size + ")\n" + "array: "
					+ Arrays.toString(elementData) + " (capacity=" + elementData.length + ")");
		}
	}
	...
	// post: returns new ArrayIntList that contains a running total of the original
	// list
	public ArrayIntList upToNowTotal() {
		ArrayIntList result = new ArrayIntList();
		int sum = 0;
		for (int index = 0; index < size; index++) {
			// increase sum value with adding elements in this class's array in the order
			sum += elementData[index];
			// add sum value in the result array
			result.add(sum);
		}
		return result;
	}

	// post: returns whether or not a list of integers is pairwise sorted
	public boolean isPairSorted() {
		int start = size / 2;
		// exclude last number from analyzing when size is odd number
		if (size % 2 != 0) {
			start--;
		}
		// check pair of number sorted correctly
		for (; start >= 0; start--) {
			if (elementData[2 * start] > elementData[2 * start + 1]) {
				return false;
			}
		}
		return true;
	...
	// pre: 0 <= n <= size
	// post:takes an integer n as a parameter and that removes the last n values
	// from a list of integers
	public void removeLast(int n) {
		// notify user that input is not valid showing message on console;
		if (n < 0 || n > size) {
			System.out.println("Not valid input. Run method again with valid input");
			return;
		}
		// remove last n number of values from the list
		size -= n;
	}
